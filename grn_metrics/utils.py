...
def get_confidence_interval(mean: float, std: float, n: int, confidence_level: int = 0.95) -> float:
    if confidence_level not in Z_STAR_MAP.keys():
        raise ValueError(f"Please provide a confidence level in: [{', '.join(Z_STAR_MAP.keys())}]")
