...
from grn_metrics.metrics.reference_nets import (
    gen_degree_preserving_network,
    gen_er_network,
)
...
def _get_network_path(network: str) -> Union[Path, None]:
    return next(
        (p for p in _get_networks() if p.name == f"{network}.nxgraph.pkl"), None
    )
...
def validate_network(
    ...
    if not network_path:
        raise click.BadParameter(
            f"Invalid network name: {value}. Make sure {value}.nxgraph.pkl exists in the networks directory."
        )
...
def _compute_reference_nets(
    network: nx.Graph, compare: str, comparison_network_n: int
) -> dict:
    ret = {}
    for reference_network_type in compare:
        reference_networks = []
        with progressbar(
            range(comparison_network_n),
            label=f"Generating {reference_network_type} networks...",
        ) as reference_networks_n:
            for _ in reference_networks_n:
                reference_networks.append(
                    REFERENCE_NET_GENERATORS[reference_network_type](network)
                )
...
def _get_metric_stats(
    ...
    for (
        ...
        reference_net_list,
    ) in reference_networks.items():  # This could be empty, btw.
        n = len(reference_net_list)
        recorded_values = np.zeros(n)
        with progressbar(
            range(n),
            f" - Computing {metric_name} for {reference_network_type} networks...",
        ) as _reference_networks:
        ...
        confidence_interval = get_confidence_interval(
            mean, std, len(recorded_values), confidence_level=confidence_level
        )
...
def _print_metric_stats(stats: dict, metric_name: str, compare: list[str]):
    ...
    for reference_network_type in compare:
        ...
        click.echo(
            f"- - Mean value: {click.style(stats[reference_network_type]['mean'], fg='green')}"
        )
        click.echo(
            f"- - Standard Deviation: {click.style(stats[reference_network_type]['std'], fg='green')}"
        )
        click.echo(
            f"- - Confidence Interval: ±{click.style(stats[reference_network_type]['confidence_interval'], fg='green')}"
        )
    click.echo("\n", nl=False)
...
def comparison_params(func: Callable):
    ...
    @click.option(
        "-cn",
        "--comparison-network-n",
        type=click.IntRange(2, 1000),
        default=1000,
        help="The number of reference networks to generate.",
    )
    @click.option(
        "-cf",
        "--confidence-level",
...
def small_worldness(
    ...
    """
    _reference_networks = _compute_reference_nets(
        network, compare, comparison_network_n
    )
    small_worldness_stats = _get_metric_stats(
        network,
        centrality.small_worldness,
        "Small-worldness",
        _reference_networks,
        confidence_level,
    )
...
def community_calculation(network: nx.Graph):
    ...
    for i, community in enumerate(network_communities):
        click.echo(
            click.style(f"Community {i + 1}: ", bold=True) + ", ".join(list(community))
        )
...
def clustering_metrics(
    ...
    """
    Get the Modularity, Global Clustering Coefficient and Average Clustering metrics, normalized against reference networks.
    """
    _reference_networks = _compute_reference_nets(
        network, compare, comparison_network_n
    )
    modularity_stats = _get_metric_stats(
        network,
        communities.get_modularity,
        "Modularity",
        _reference_networks,
        confidence_level,
    )
    global_clustering_coefficient_stats = _get_metric_stats(
        network,
        communities.get_global_clustering_coefficient,
    ...
    average_clustering_stats = _get_metric_stats(
        network,
        communities.get_average_clustering,
        "Average Clustering",
        _reference_networks,
        confidence_level,
    )
    ...
    _print_metric_stats(
        global_clustering_coefficient_stats, "Global Clustering Coefficient", compare
    )
...
def degree_metrics(
    ...
    """
    Get the Average Degree and Characteristic Path Length of the network, normalized against reference networks.
    """
    _reference_networks = _compute_reference_nets(
        network, compare, comparison_network_n
    )
    average_degree_stats = _get_metric_stats(
        network,
        centrality.get_average_degree,
        "Average Degree",
        _reference_networks,
        confidence_level,
    )
    characteristic_path_length_stats = _get_metric_stats(
        network,
        centrality.get_characteristic_path_length,
    ...
    _print_metric_stats(
        characteristic_path_length_stats, "Characteristic Path Length", compare
    )


@metrics.command("degrees")
@click.argument("network", callback=validate_network)
def degrees(network: nx.Graph):
    """
    Print the node degrees.
    """
    for node in network.nodes:
        print(f"{node}: {network.degree[node]}")


@metrics.command("rich-club-coefficient")
@click.argument("network", callback=validate_network)
@click.option(
    "-o", "--output", type=click.Path(), help="Path to the CSV file to save to"
)
@click.option(
    "-n",
    "--n-rand",
    type=int,
    help="The number of random reference networks",
    default=1000,
)
def rich_club_coefficient(network: nx.Graph, output: click.Path, n_rand: int):
    ...
    if not output:
        ...
        click.echo(
            f"Exported the Rich Club Coefficients to: "
...
def centrality_metrics(network: nx.Graph, output: click.Path):
    ...
    click.echo(
        f"Exported the centrality metrics to: " + click.style(output, fg="green")
    )
...
@click.option(
    "-c",
    "--colours",
    type=click.Choice(["Communities"]),
    default=None,
    help="The colouring of the nodes in the graph.",
)
